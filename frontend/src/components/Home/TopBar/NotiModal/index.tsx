...
const NotiModal = ({ listNoti, getListNotify, setActiveNotiModal }: INotiModal) => {  
  ...
  const friendAccept = async (id: string, uid: string, nickname: string, avatar: string) => {
    try {
        ...
        listNoti.length > 0 ? 
          ...
          {listNoti.map((data: any, index: number) => (
            <S.NotiItem key={index}>
              ...
              <S.NotiAccept onClick={() => friendAccept(data._id, data.uid, data.name, data.avatar)}>Accept</S.NotiAccept>
              <S.NotiDecline onClick={() => friendDecline(data._id)}>Decline</S.NotiDecline>
            </S.NotiItem>
          ))}
        </S.NotiList> : <S.NotiText>
